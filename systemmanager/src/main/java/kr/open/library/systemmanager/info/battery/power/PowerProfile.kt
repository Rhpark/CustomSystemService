package kr.open.library.systemmanager.info.battery.power
...
public class PowerProfile(private val context: Context) {
    ...
    private val powerProfileInstance: Any? by lazy {
        safeCatch("createPowerProfileInstance", null) {
     ...
     */
    public fun getAveragePower(type: PowerProfileVO): Any? = safeCatch("getAveragePower", null) {
     ...
     * @return 평균 전력 소비량, 오류 발생 시 null
     */
    public fun getAveragePower(type: PowerProfileVO, index: Int): Any? = safeCatch("getAveragePower", null) {
        if (!isPowerProfileAvailable()) {
     ...
     */
    public fun getBatteryCapacity(): Double = safeCatch("getBatteryCapacity", DEFAULT_BATTERY_CAPACITY) {
