package kr.open.library.systemmanager.info.connectivity
...
public class NetworkConnectivityInfo(context: Context) : BaseSystemService(
     ...
     */
    public val wifiController: WifiController by lazy { 
        WifiController(context) 
    }
    ...
    /**
     * 네트워크 연결 여부 확인 (Result 패턴)
     * Check if network is connected (Result pattern)
     */
    @RequiresPermission(ACCESS_NETWORK_STATE)
     ...
     */
    @RequiresPermission(ACCESS_NETWORK_STATE)
    public fun getNetworkCapabilities(): NetworkCapabilities? = getCapabilitiesInternal().getOrNull()
    
    /**
     * 현재 네트워크의 NetworkCapabilities 반환 (Result 패턴)
     * Get NetworkCapabilities of current network (Result pattern)
     */
    @RequiresPermission(ACCESS_NETWORK_STATE)
    ...
    /**
     * 현재 네트워크의 LinkProperties 반환 (Result 패턴)
     * Get LinkProperties of current network (Result pattern)
     */
    @RequiresPermission(ACCESS_NETWORK_STATE)
